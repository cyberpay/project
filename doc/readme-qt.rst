Eryllium-qt: Qt4 GUI for Eryllium
===============================
...
Alternatively, install Qt Creator and open the `Eryllium-qt.pro` file.

An executable named `Eryllium-qt` will be built.
...
A warning for people using the *static binary* version of Eryllium on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Eryllium is linked against libdb5.3.
...
installed causes Eryllium-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
