...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ErylliumTEST2\r");
            Send(hSocket, "WHO #ErylliumTEST2\r");
        } else {
            // randomly join #Eryllium00-#Eryllium05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Eryllium%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Eryllium%02d\r", channel_number).c_str());
        }
